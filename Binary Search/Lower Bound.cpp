...
int32_t main()
{
    ...
    same answer.But if not it will given bigger next
    element*/
    ...
    /*if using array
    so->
    lower_bound(starting address,total range previous elemnet,which element)
    starting address = -> which element to start find lower bound */
    ...
    int *ptr = lower_bound(arra, arra+n, findd);
    ///cout<<(*ptr)<<endl;
...
int32_t main()
{
    ...
    cout<<endl;
    /// 4 5 5 7 8 25
    int *ptr = lower_bound(arra, arra+n, 5);
    ///cout<<(*ptr)<<endl;
